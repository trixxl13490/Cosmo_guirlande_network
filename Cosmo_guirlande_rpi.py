    ...
    def run(self):
        ...
        try:
            while True:
                ...
                ######################################################################################################################
# wait for animation type and threshold
                if self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,manual") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,manual"):
                    print("manual control, do nothing while checkbox is on")
                    # time.sleep(0.3)

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,strombo") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,strombo"):
                    self.state = "strombo"
                    self.stromboscope(self.color1, 0.05)
                    # time.sleep(0.3)

                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,color1') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,color1'):
                    self.state = 'color1'
                    try:
                        function_type, function, self.color1 = self.newSocket_mqtt.data_rcv.split(',')
                    except :
                        function_type, function, self.color1 = self.newSocket_mqtt_mqtt.data_rcv.split(',')
                ...
                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,color2') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,color2'):
                    self.state = 'color2'
                    try:
                        function_type, function, self.color2 = self.newSocket_mqtt.data_rcv.split(',')
                    except:
                        function_type, function, self.color2 = self.newSocket_mqtt_mqtt.data_rcv.split(',')
                ...
                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,rainbow") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,rainbow") :
                    self.state = "rainbow"
                    for j in range(2):
                        self.rainbow_cycle(0.01)

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,blackout") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,blackout"):
                    self.state = "blackout"
                    self.blackout()

                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,chase') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,chase'):
                    self.state = "chase"
                    try:
                        function_type, function, chase_speed, chase_size = self.newSocket_mqtt.data_rcv.split(',')
                    except :
                ...
                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,comet') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,comet'):
                    self.state = "comet"
                ...
                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,sparkle') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,sparkle'):
                    self.state = "sparkle"
                ...
                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,pulse') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,pulse'):
                    self.state = "pulse"
                ...
                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,solid') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,solid'):
                    self.state = "solid"
                    self.solid()
                    time.sleep(0.5)

                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,colorcycle') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,colorcycle'):
                    self.state = "colorcycle"
                ...
                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,dancingPiScroll') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,dancingPiScroll'):
                    self.state = "dancingPiScroll"
                    self.blackout()
                    self.dancingPiScroll()

                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,dancingPiEnergy') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,dancingPiEnergy'):
                    self.state = "dancingPiEnergy"
                    self.blackout()
                    self.dancingPiEnergy()

                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,dancingPiSpectrum') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,dancingPiSpectrum'):
                    self.state = "dancingPiSpectrum"
                    self.blackout()
                    self.dancingPiSpectrum()

                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,stop_dancingPiScroll') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,stop_dancingPiScroll'):
                    self.state = "stop_dancingPiScroll"
                    self.stop_dancingPiScroll()

                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,stop_dancingPiEnergy') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,stop_dancingPiEnergy'):
                    self.state = "stop_dancingPiEnergy"
                    self.stop_dancingPiEnergy()

                elif self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,stop_dancingPiSpectrum') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,stop_dancingPiSpectrum'):
                    self.state = "stop_dancingPiSpectrum"
                    self.stop_dancingPiSpectrum()

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,R") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,R') :
                    self.state = "R"
                    function_type, function, self.r = self.newSocket_mqtt.data_rcv.split(',')
                    self.changeColor(self.r, self.g, self.b, self.w)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,G") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,G'):
                    self.state = "G"
                    function_type, function, self.g = self.newSocket_mqtt.data_rcv.split(',')
                    self.changeColor(self.r, self.g, self.b, self.w)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,B") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,B'):
                    self.state = "B"
                    function_type, function, self.b = self.newSocket_mqtt.data_rcv.split(',')
                    self.changeColor(self.r, self.g, self.b, self.w)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,W") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,W'):
                    self.state = "W"
                    function_type, function, self.w = self.newSocket_mqtt.data_rcv.split(',')
                    self.changeColor(self.r, self.g, self.b, self.w)
                    # time.sleep(0.5) 
                
                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,colorAll2Color") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,colorAll2Color'):
                    self.state = "colorAll2Color"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.colorAll2Color((int(self.r), int(self.g), int(self.b)), (255,165,0))
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,FadeInOut") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,FadeInOut'):
                    self.state = "FadeInOut"
                    function_type, function= self.newSocket_mqtt.data_rcv.split(',')
                    self.FadeInOut(self.r, self.g, self.b, 0)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,Strobe") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,Strobe'):
                    self.state = "Strobe"
                    function_type, function= self.newSocket_mqtt.data_rcv.split(',')
                    self.Strobe(self.r, self.g, self.b,  10, 0, 1)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,HalloweenEyes") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,HalloweenEyes'):
                    self.state = "HalloweenEyes"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.HalloweenEyes(self.r, self.g, self.b, 1, 1, True, 10, 1, 3)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,CylonBounce") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,CylonBounce'):
                    self.state = "CylonBounce"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.CylonBounce(self.r, self.g, self.b, 2, 0, 0)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,NewKITT") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,NewKITT'):
                    self.state = "NewKITT"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.NewKITT(self.r, self.g, self.b, 4, 0, 0)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,Twinkle") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,Twinkle'):
                    self.state = "Twinkle"
                    function_type, function= self.newSocket_mqtt.data_rcv.split(',')
                    self.Twinkle(self.r, self.g, self.b, 10, 0.1, False)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,TwinkleRandom") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,TwinkleRandom'):
                    self.state = "TwinkleRandom"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.TwinkleRandom(20, 0.1, False)
                    # time.sleep(0.5) 
 
                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,SnowSparkle") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,SnowSparkle'):
                    self.state = "SnowSparkle"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.SnowSparkle(self.r, self.g, self.b, 100, 0.1, 0.3 )
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,*RunningLights") or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,*RunningLights'):
                    self.state = "RunningLights"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.RunningLights(self.r, self.g, self.b, 0)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,colorWipe") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,colorWipe"):
                    self.state = "colorWipe"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.colorWipe(self.r, self.g, self.b, 0 )
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,theaterChaseRainbow") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,theaterChaseRainbow")  :
                    self.state = "theaterChaseRainbow"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.theaterChaseRainbow(0.1, 30)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,Fire") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,Fire"):
                    self.state = "Fire"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    while self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,Fire') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,Fire'):
                        self.Fire(55, 120,0, 100)

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,FireCustom") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,FireCustom"):
                    self.state = "FireCustom"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    while self.newSocket_mqtt.data_rcv.startswith('cosmoguirlande,FireCustom') or  self.newSocket_mqtt_mqtt.data_rcv.startswith('cosmoguirlande,FireCustom'):
                        self.FireCustom(self.r, self.g, self.b )

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,meteorRain") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,meteorRain") :
                    self.state = "meteorRain"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.meteorRain(self.r, self.g, self.b, 10, 64, True, 1, 0 )

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,fadeToBlack") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,fadeToBlack"):
                    self.state = "fadeToBlack"
                    function_type, function= self.newSocket_mqtt.data_rcv.split(',')
                    #self.fadeToBlack(self.r, self.g, self.b)

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,*BouncingBalls") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,*BouncingBalls"):
                    self.state = "BouncingBalls"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.BouncingBalls(255, 0, 0, 3, 100) 
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,*BouncingColoredBalls") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,*BouncingColoredBalls") :
                    self.state = "BouncingColoredBalls"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.BouncingColoredBalls(3, ((255,0,0),(0,255,0),(0,0,255)), 1000)

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,Matrix") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,Matrix"):
                    self.state = "Matrix"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.pixels.fill((0, 0, 0))
                    self.pixels.show()
                ...
                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,*Drain") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,*Drain"):
                    self.state = "Drain"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.drain(self.levelobjcount, 0)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,Pancake") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,Pancake"):
                    self.state = "Pancake"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.pancake(self.levelgroups, 0)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,HeartBeat") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,HeartBeat"):
                    self.state = "HeatBeat"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    # HeartBeat(red, green, blue, cycles):
                    self.HeartBeat(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255), 2)
                ...
                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,rainbowWithGlitter") or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,rainbowWithGlitter"):
                    self.state = "rainbowWithGlitter"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.rainbowWithGlitter(0, 7, 0, 100)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,Confetti")  or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,Confetti"):
                    self.state = "Confetti"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.confetti(0.1, 1000)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,Sinelon")  or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,Sinelon"):
                    self.state = "Sinelon"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.sinelon(0, 230, 0, 500)
                    # time.sleep(0.5) 

                elif self.newSocket_mqtt.data_rcv.startswith("cosmoguirlande,**BPM")  or  self.newSocket_mqtt_mqtt.data_rcv.startswith("cosmoguirlande,**BPM"):
                    self.state = "BPM"
                    function_type, function = self.newSocket_mqtt.data_rcv.split(',')
                    self.bpm(self.PartyColors_p, 0, 50)
